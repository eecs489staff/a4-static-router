...
from ltprotocol.ltprotocol.ltprotocol import LTMessage, LTProtocol, LTTwistedServer
...
def strip_null_chars(s):
    """Remove null characters from a a bytes object."""
    cleaned_bytes = b""
    for b in s:
        if b != 0:
...
class VNSInterface:
    ...
    def pack(self):
        logger.info(
            f"Types: name={type(self.name)} mac={type(self.mac)} ip={type(self.ip)} mask={type(self.mask)}"
        )

        return struct.pack(VNSInterface.FORMAT,
                           VNSInterface.HWINTERFACE, self.name,
                           VNSInterface.HWSPEED, 0, b'',
                           VNSInterface.HWETHER, self.mac,
                           VNSInterface.HWETHIP, self.ip, b'',
                           VNSInterface.HWSUBNET, 0, b'',
                           VNSInterface.HWMASK, self.mask, b'')
...
class VNSHardwareInfo(LTMessage):
    ...
    def to_msg(self):
        return f'Hardware Info: { " || ".join([str(intf) for intf in self.interfaces]) }'
...
class VNSOpenTemplate(LTMessage):
    ...
    def pack(self):
        body = b''.join((inet_aton(ip) + struct.pack('>B', mask)) for ip,mask in self.src_filters)
...
class VNSAuthReply(LTMessage):
    ...
    def __str__(self):
        return 'AUTH_REPLY: ' + ' username=' + self.username.decode('utf-8')
