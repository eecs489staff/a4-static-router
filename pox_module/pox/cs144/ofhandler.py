...
class OFHandler (EventMixin):
  ...
  def _handle_SRPacketOut(self, event):
    logger.info("SRPacketOut on port {}, pkt={}", event.port, event.pkt)
...
class cs144_ofhandler (EventMixin):
  ...
  def _handle_ConnectionUp (self, event):
    logger.debug("Connection %s" % (event.connection,))
...
def get_ip_setting():
  ...
  for line in f:
    ...
    name, ip = line.split()
    if ip == "<ELASTIC_IP>":
      logger.info("ip configuration is not set, please put your Elastic IP addresses into %s" % IPCONFIG_FILE)
      sys.exit(2)
    #print name, ip
    IP_SETTING[name] = ip
...
def launch(transparent=False):
  ...
  r = get_ip_setting()
  if r == -1:
    logger.debug("Couldn't load config file for ip addresses, check whether %s exists" % IPCONFIG_FILE)
    sys.exit(2)
  else:
    logger.debug('*** ofhandler: Successfully loaded ip settings for hosts\n %s\n' % IP_SETTING)
